...
function triggerMalevolentShrine() {
  ...
  // ── Environment: Sky, Water, Rocks ──
  const skyGeo = new THREE.SphereGeometry(300, 128, 128);
  ...
  const water = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ color: 0x440000, metalness: 0.9, roughness: 0.1 })
  );
  ...
  const shrineRed = new THREE.MeshPhysicalMaterial({ color: 0xaa0000, emissive: 0x330000, roughness: 0.4, metalness: 0.3 });
  const roofMat = new THREE.MeshPhysicalMaterial({ color: 0x111111, roughness: 0.8, metalness: 0.1 });
  ...
  for (let i = 0; i < 5; i++) {
    ...
    shrine.add(spike);
    const skull = new THREE.Mesh(new THREE.SphereGeometry(0.4, 16, 16), new THREE.MeshStandardMaterial({ color: 0xdddddd }));
  ...
  [-1, 1].forEach(s => {
    const fang = new THREE.Mesh(new THREE.ConeGeometry(0.35, 1.5, 8), new THREE.MeshStandardMaterial({ color: 0xffffff }));
  ...
  function animate() {
    ...
    teeth.forEach((tooth, i) => {
      tooth.userData.angle += 0.015;
      tooth.position.set(Math.cos(tooth.userData.angle) * tooth.userData.radius, 1 + Math.sin(t + i) * 2, Math.sin(tooth.userData.angle) * tooth.userData.radius);
      tooth.lookAt(0, 0, 0);
    });

    mouth.rotation.z += 0.04;
    mouth.scale.setScalar(1 + Math.sin(t * 8) * 0.08);

    const pos = water.geometry.attributes.position;
    for (let i = 0; i < pos.count; i++) {
      pos.setZ(i, Math.sin(pos.getX(i) * 0.08 + t * 4) + Math.cos(pos.getY(i) * 0.08 + t * 3));
    }
